...
function repeatString(str, times) {
  const re = times >= 0 ? times : 0;
  return str.repeat(re);
}
...
function removeFirstOccurrences(str, value) {
  ...
  return str.indexOf(value) !== -1 ? first + second : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  return str.lastIndexOf(value) !== -1 ? first + second : str;
}
...
function formatTime(minutes, seconds) {
  const resm = `${minutes}`.padStart(2, '0');
  const ress = `${seconds}`.padStart(2, '0');
  return `${resm}:${ress}`;
}
...
function countVowels(str) {
  return str.replaceAll(/[^aeiouy]/gi, '').length;
}
...
function isPalindrome(str) {
  const res = str.replaceAll(/[ ,!.?]/gi, '').toLowerCase();
...
function reverseWords(str) {
  const arr = str.split(' ');
  const res = arr.map((x) => x.split('').reverse().join(''));
  return res.join(' ');
}
...
function invertCase(str) {
  const arr = str.split('');
  const res = arr.map((x) =>
    /[A-Z]/g.test(x) ? x.toLowerCase() : x.toUpperCase()
  );
  return res.join('');
}
...
function getCardId(value) {
  ...
  const num = [value.slice(0, -1), value.slice(-1)];
  return cards.indexOf(num[0]) + suits.indexOf(num[1]) * 13;
}
